...
# PyTorch 2.6+ changed weights_only default to True, causing issues with fairseq/RVC checkpoints
# Add fairseq's Dictionary class to safe globals to allow loading
try:
...
import faiss
from .root_pipe import VC, change_rms, bh, ah
import librosa
    ...
    def infer(
        ...
        if isinstance(input_audio_path, tuple):
            ...
            if source_sr != base_sr:
                audio = librosa.resample(
                    ...
                    orig_sr=source_sr,
                    target_sr=base_sr,
                    res_type='kaiser_best'  # Explicit res_type for librosa 0.10+ compatibility
                )
        ...
        if resample_sr >= 16000 and tgt_sr != resample_sr:
            audio_opt = librosa.resample(
                audio_opt, orig_sr=tgt_sr, target_sr=resample_sr, res_type='kaiser_best'  # Explicit res_type for librosa 0.10+ compatibility
            )
